            ...
            for schema in conn.catalog.get_schema_names():
                for table in conn.catalog.get_table_names(schema):
                    ...
                    for c in table_def.columns:
                        col = clean(str(c.name))
                        cols.append(col)
                        column_table_map.setdefault(col.lower(), []).append(table_name)
...
# ============================================================
# STEP 2: RELATIONSHIPS EXTRACTION
# ============================================================
def extract_relationships(root, column_table_map, tables):
    relationships = []
    seen = set()
    ...
    # --------------------------------------------------------
    # 1️⃣ RELATIONSHIPS NODE (Logical Relationships)
    # --------------------------------------------------------
    for rel in root.findall(".//datasource//relationships//relationship"):
        expr = rel.find("expression")
        if expr is None:
            continue
        ...
        left, right = cols
        lt = column_table_map.get(left.lower(), [])
        rt = column_table_map.get(right.lower(), [])

        if lt and rt:
            add(lt[0], left, rt[0], right)

    # --------------------------------------------------------
    # 2️⃣ JOIN-BASED RELATIONSHIPS (MOST COMMON)
    # --------------------------------------------------------
    for join in root.findall(".//join"):
        ...
        lt = column_table_map.get(left_col.lower(), [])
        rt = column_table_map.get(right_col.lower(), [])

        if lt and rt:
            add(lt[0], left_col, rt[0], right_col)

    # --------------------------------------------------------
    # 3️⃣ FALLBACK: COMMON COLUMN HEURISTIC (CASE-INSENSITIVE)
    # --------------------------------------------------------
    if not relationships:
        log.warning("No explicit relationships found, using fallback heuristic")
